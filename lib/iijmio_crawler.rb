...
module IijmioCrawler
  ...
  # 契約情報の取得
  def self.scan_contract(user_data)
  ...
  def self.scan_packet_usage(user_data)
    usage_array_index = 0
  ...
  def self.scan_coupon_rest(user_data)
    ...
    total = numeric_xpath(@agent.page, coupon_total_xpath)
    this_month = numeric_xpath(@agent.page, coupon_this_month_xpath)
    prev_month = numeric_xpath(@agent.page, coupon_prev_month_xpath)
  ...
  def self.numeric_xpath(page, xpath)
